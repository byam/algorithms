...
void solve() {
    ...
    segtree<int, opmin, emin> segMin(a);
    segtree<int, opmax, emax> segMax(a);

    for (int i = 0; i < m; i++) {
        int l, r, x;
        read(l, r, x);
        l--;
        int mi = segMin.prod(l, r);
        int ma = segMax.prod(l, r);
        ...
        if (mi == ma and mi == x) {
            ...
            // pair
            int ans1 =
                lower_bound(p.begin(), p.end(), make_pair(k, l)) - p.begin();
            cout << p[ans1].second + 1 << endl;
        }
    }
}
