...
bool repeatedSubstringPattern(string s) {
    int n = s.length();
    vector<int> lps = initPrefixTable(s);
    // if a string repeats a length 5 substring 4 times,
    // then the last entry would be of value 15.
    return lps[n - 1] and lps[n - 1] % (n - lps[n - 1]) == 0;
}

int main() {
    out(repeatedSubstringPattern("abab"));
    out(repeatedSubstringPattern("aba"));
    out(repeatedSubstringPattern("abcabcabcabc"));
    return 0;
}
