...
int countVowelSubstrings(string word) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 4; j < n; j++) {
            ...
            int k = s.length();
            // out(s);
            if (s.find('a') < k and s.find('e') < k and s.find('i') < k and
                s.find('o') < k and s.find('u') < k) {
                bool found = true;
                for (auto c : s) {
                    bool isLowercaseVowel = (c == 'a' || c == 'e' || c == 'i' ||
                                             c == 'o' || c == 'u');
                    if (!isLowercaseVowel) found = false;
                }
                if (found) ans++;
            }
        }
...
void solve() {
    // in
    out(countVowelSubstrings("aeiouu"));
    out(countVowelSubstrings("cuaieuouac"));
}
