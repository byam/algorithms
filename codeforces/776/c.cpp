...
bool sortw(vi& a, vi& b) { return a[0] < b[0]; }
bool sortx(vi& a, vi& b) { return a[1] < b[1]; }
...
void solve() {
    ...
    while (l < r) {
        if (p[l][0] == INT_MAX)
            l++;
        else if (p[r][0] == INT_MAX)
            r--;
        else {
            sum += p[l][0] + p[r][0];
            ans.push_back({p[l][2] + 1, p[r][2] + 1});
            l++;
            r--;
        }
    }
    out(sum);
    for (auto [x, y] : ans) outt(x, y);
}
