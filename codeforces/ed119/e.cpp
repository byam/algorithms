...
void update(map<int, vector<int>>& hashmap, vector<int>& q) {
    // Iterating with q in given query

    if (hashmap.count(q[1]))
        hashmap[q[1]].insert(hashmap[q[1]].end(), hashmap[q[0]].begin(),
                             hashmap[q[0]].end());
    else
...
void solve() {
    ...
    for (auto it = hashmap.begin(); it != hashmap.end(); ++it) {
        for (auto index : it->second) ans[index] = it->first;
    }
