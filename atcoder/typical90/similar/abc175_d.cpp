...
/*-----------------------------------
        Coding Starts Here
------------------------------------*/
int cycle_step(vi nxt, ll st, ll k_step, ll mod, vi cost) {
    // cycle 計算
    ...
    // big k を cycle で小さくする
    if (k_step >= time_stamp[pos]) {
        k_step = (k_step - time_stamp[pos]) % cycle + time_stamp[pos];
    }

    // 更新後は、最初から k_step が回答
    for (int i = 1; i <= mod; ++i) {
        if (time_stamp[i] <= k_step) {
            answer += cost[i];
        }
    }
    return answer;
...
void solve() {
    ...
    for (int st = 1; st <= n; st++) {
        ll cost = cycle_step(p, st, k, n + 1, c);
        chmax(ans, cost);
    }
    out(ans);
}
